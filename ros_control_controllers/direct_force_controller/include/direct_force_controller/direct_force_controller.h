 ...
 *
 * Title:   direct_force_controller.h
 * Author:  Sabatino Simeone, Giovanni Longobardi
 * Org.:    UNISA
 * Date:    Dec 20, 2019
 *
 * This class implements a direct force controller with inner
 * position loop based on ros_control
 *
 * -------------------------------------------------------------------
 */

#ifndef INCLUDE_DIRECT_FORCE_CONTROLLER_DIRECT_FORCE_CONTROLLER_H
#define INCLUDE_DIRECT_FORCE_CONTROLLER_DIRECT_FORCE_CONTROLLER_H
...
class DirectForceController : public controller_interface::Controller<hardware_interface::PositionJointInterface>

{
  /**
   * @brief Enumerator defining the states of the direct force controller
   */
  enum State
  {
    independent_joint_control,
    static_direct_force_control,
    trajectory_execution_direct_force_control
  };
...
public:
  ...
  static const unsigned int direct_force_control_dof = 3;

  DirectForceController();
  virtual ~DirectForceController();
  ...
  /**
    * @throw direct_force_controller::Exception if no valid solution to the kinematic inversion is found
    */
...
private:
  ...
  /**
    ...
    * 
    * @throw direct_force_controller::Exception if the JMG contains joint types that are not currently supported
    */
  ...
  // Force control
  bool force_control_along_x_;
  bool force_control_along_y_;
  bool force_control_along_z_;
  ...
  Eigen::Vector3d x_c_; // Workspace command in the end-effector frame
...
#endif /* INCLUDE_DIRECT_FORCE_CONTROLLER_DIRECT_FORCE_CONTROLLER_H */
